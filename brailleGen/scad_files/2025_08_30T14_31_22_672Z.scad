...
union() {
        ...
        translate([line_number * plate_depth,0,0]) {
            for (count = [0:len(chars) - 1]) {
                rotate([0,0,270])
                translate([leg+(top_width/2),0,depth])
                //translate([4*block_width/6,((4*leg+3*top_width)-plate_length)/2,depth])
...
text_rectangle (depth, text_block_width, text_block_length, "text_input = ["output "hi""];"); // Block with Text
